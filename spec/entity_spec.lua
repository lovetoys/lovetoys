...
describe('Entity', function()
    ...
    it(':add() adds a Component', function()
        ...
        assert.are.equal(entity.components[testComponent.__name], testComponent)
    end)

    it(':add() doesn`t add the same Component twice', function()
        testComponent.int = 12
        entity:add(testComponent)
        assert.are.equal(entity.components[testComponent.__name].int, 12)
        -- Creation of new testComponent with varying variables
        testComponent = TestComponent()
        testComponent.int = 13
        entity:add(testComponent)
        assert.are_not.equal(entity.components[testComponent.__name].int, 13)
    end)

    it(':get() gets a Component', function()
        entity:add(testComponent)
        assert.are.equal(entity:get(testComponent.__name), testComponent)
    end)
    ...
    it(':set() adds and overwrites Components', function()
        ...
        testComponent.int = 13
        entity:set(testComponent)
        assert.are.equal(entity.components[testComponent.__name].int, 13)
    end)
    ...
    it(':addMultiple() adds Multiple Components at once', function()
        ...
        assert.are.equal(entity.components[testComponent1.__name], testComponent1)
        assert.are.equal(entity.components[testComponent2.__name], testComponent2)
        assert.are.equal(entity.components[testComponent3.__name], testComponent3)
    end)
